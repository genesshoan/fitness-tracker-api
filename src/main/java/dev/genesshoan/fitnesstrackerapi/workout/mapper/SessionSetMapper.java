...
public interface SessionSetMapper {
    ...
    @Mapping(
        target = "achievements",
        expression = "java(getAchievements(set.getId(), achievementsBySetId))"
    )
    @Named("withAchievements")
    SessionSetResponseDTO toResponseDTO(
        SessionSet set,
        @Context Map<UUID, List<AchievementDTO>> achievementsBySetId
    );
