...
import dev.genesshoan.fitnesstrackerapi.AbstractWebMvcTestNoSecurity;
...
class AuthControllerTest extends AbstractWebMvcTestNoSecurity {
    ...
    void refreshShouldReturn200AndNewTokens() throws Exception {
        ...
        mockMvc
            .perform(
                post("/api/v1/auth/refresh").header(
    ...
    void logoutShouldReturn204() throws Exception {
        mockMvc
            .perform(
                delete("/api/v1/auth/logout").header(
                    HttpHeaders.AUTHORIZATION,
                    "Bearer access-token"
                )
            )
